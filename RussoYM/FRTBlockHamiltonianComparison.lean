...
/-! ### Concrete `SU(2)` electric directions

The following three curves are the exponentials of `i` times the Pauli
matrices, written directly with sine and cosine.  Writing them explicitly
avoids relying on the presently absent general theorem
`det (Matrix.exp X) = exp (Matrix.trace X)` while still proving, rather than
assuming, that the curves lie in `SU(2)`. -/

/-- The matrix curve generated by `i σ₁`. -/
noncomputable def su2PauliXMatrix (t : Real) : Matrix (Fin 2) (Fin 2) Complex :=
  !![(Real.cos t : Complex), Complex.I * Real.sin t;
     Complex.I * Real.sin t, (Real.cos t : Complex)]

/-- The matrix curve generated by `i σ₂`. -/
noncomputable def su2PauliYMatrix (t : Real) : Matrix (Fin 2) (Fin 2) Complex :=
  !![(Real.cos t : Complex), (Real.sin t : Complex);
     (-(Real.sin t) : Complex), (Real.cos t : Complex)]

/-- The matrix curve generated by `i σ₃`. -/
noncomputable def su2PauliZMatrix (t : Real) : Matrix (Fin 2) (Fin 2) Complex :=
  !![(Real.cos t : Complex) + Complex.I * Real.sin t, 0;
     0, (Real.cos t : Complex) - Complex.I * Real.sin t]

set_option linter.flexible false in
theorem su2PauliXMatrix_mem (t : Real) :
    su2PauliXMatrix t ∈ Matrix.specialUnitaryGroup (Fin 2) Complex := by
  ...
  · rw [Matrix.mem_unitaryGroup_iff]
    ...
    all_goals apply Complex.ext <;>
      norm_num [su2PauliXMatrix, Matrix.mul_apply, star,
        Complex.cos_ofReal_re, Complex.sin_ofReal_re]
    all_goals nlinarith [Real.sin_sq_add_cos_sq t]
  · simp [su2PauliXMatrix, Matrix.det_fin_two]
    apply Complex.ext <;>
      norm_num [Complex.cos_ofReal_re, Complex.sin_ofReal_re]
    nlinarith [Real.sin_sq_add_cos_sq t]

theorem su2PauliYMatrix_mem (t : Real) :
    su2PauliYMatrix t ∈ Matrix.specialUnitaryGroup (Fin 2) Complex := by
  rw [Matrix.mem_specialUnitaryGroup_iff]
  constructor
  · rw [Matrix.mem_unitaryGroup_iff]
    ext i j
    fin_cases i <;> fin_cases j
    all_goals apply Complex.ext <;>
      norm_num [su2PauliYMatrix, Matrix.mul_apply, star,
        Complex.cos_ofReal_re, Complex.sin_ofReal_re]
    all_goals nlinarith [Real.sin_sq_add_cos_sq t]
  · simp [su2PauliYMatrix, Matrix.det_fin_two]
    norm_cast
    nlinarith [Real.sin_sq_add_cos_sq t]

set_option linter.flexible false in
theorem su2PauliZMatrix_mem (t : Real) :
    su2PauliZMatrix t ∈ Matrix.specialUnitaryGroup (Fin 2) Complex := by
  rw [Matrix.mem_specialUnitaryGroup_iff]
  constructor
  · rw [Matrix.mem_unitaryGroup_iff]
    ext i j
    fin_cases i <;> fin_cases j
    all_goals apply Complex.ext <;>
      norm_num [su2PauliZMatrix, Matrix.mul_apply, star,
        Complex.cos_ofReal_re, Complex.sin_ofReal_re]
    all_goals nlinarith [Real.sin_sq_add_cos_sq t]
  · simp [su2PauliZMatrix, Matrix.det_fin_two]
    apply Complex.ext <;>
      norm_num [Complex.cos_ofReal_re, Complex.sin_ofReal_re]
    · nlinarith [Real.sin_sq_add_cos_sq t]
    · ring

noncomputable def su2PauliXCurve (t : Real) :
    Matrix.specialUnitaryGroup (Fin 2) Complex :=
  ⟨su2PauliXMatrix t, su2PauliXMatrix_mem t⟩

noncomputable def su2PauliYCurve (t : Real) :
    Matrix.specialUnitaryGroup (Fin 2) Complex :=
  ⟨su2PauliYMatrix t, su2PauliYMatrix_mem t⟩

noncomputable def su2PauliZCurve (t : Real) :
    Matrix.specialUnitaryGroup (Fin 2) Complex :=
  ⟨su2PauliZMatrix t, su2PauliZMatrix_mem t⟩

@[simp] theorem su2PauliXCurve_zero : su2PauliXCurve 0 = 1 := by
  ext i j
  fin_cases i <;> fin_cases j <;> simp [su2PauliXCurve, su2PauliXMatrix]

@[simp] theorem su2PauliYCurve_zero : su2PauliYCurve 0 = 1 := by
  ext i j
  fin_cases i <;> fin_cases j <;> simp [su2PauliYCurve, su2PauliYMatrix]

@[simp] theorem su2PauliZCurve_zero : su2PauliZCurve 0 = 1 := by
  ext i j
  fin_cases i <;> fin_cases j <;> simp [su2PauliZCurve, su2PauliZMatrix]

theorem su2PauliXCurve_add (s t : Real) :
    su2PauliXCurve (s + t) = su2PauliXCurve s * su2PauliXCurve t := by
  apply Subtype.ext
  ext i j
  fin_cases i <;> fin_cases j
  all_goals apply Complex.ext <;>
    norm_num [su2PauliXCurve, su2PauliXMatrix, Matrix.mul_apply,
      Real.sin_add, Real.cos_add, Complex.cos_ofReal_re, Complex.sin_ofReal_re]
  all_goals ring

theorem su2PauliYCurve_add (s t : Real) :
    su2PauliYCurve (s + t) = su2PauliYCurve s * su2PauliYCurve t := by
  apply Subtype.ext
  ext i j
  fin_cases i <;> fin_cases j
  all_goals apply Complex.ext <;>
    norm_num [su2PauliYCurve, su2PauliYMatrix, Matrix.mul_apply,
      Real.sin_add, Real.cos_add, Complex.cos_ofReal_re, Complex.sin_ofReal_re]
  all_goals ring

theorem su2PauliZCurve_add (s t : Real) :
    su2PauliZCurve (s + t) = su2PauliZCurve s * su2PauliZCurve t := by
  apply Subtype.ext
  ext i j
  fin_cases i <;> fin_cases j
  all_goals apply Complex.ext <;>
    norm_num [su2PauliZCurve, su2PauliZMatrix, Matrix.mul_apply,
      Real.sin_add, Real.cos_add, Complex.cos_ofReal_re, Complex.sin_ofReal_re]
  all_goals ring

set_option linter.flexible false in
theorem su2PauliXCurve_continuous : Continuous su2PauliXCurve := by
  apply Continuous.subtype_mk
  apply continuous_pi
  intro i
  apply continuous_pi
  intro j
  fin_cases i <;> fin_cases j <;> simp [su2PauliXMatrix] <;> fun_prop

set_option linter.flexible false in
theorem su2PauliYCurve_continuous : Continuous su2PauliYCurve := by
  apply Continuous.subtype_mk
  apply continuous_pi
  intro i
  apply continuous_pi
  intro j
  fin_cases i <;> fin_cases j <;> simp [su2PauliYMatrix] <;> fun_prop

set_option linter.flexible false in
theorem su2PauliZCurve_continuous : Continuous su2PauliZCurve := by
  apply Continuous.subtype_mk
  apply continuous_pi
  intro i
  apply continuous_pi
  intro j
  fin_cases i <;> fin_cases j <;> simp [su2PauliZMatrix] <;> fun_prop

/-- The three concrete Pauli one-parameter subgroups of `SU(2)`. -/
noncomputable def su2PauliElectricDirections :
    Fin 3 → WilsonElectricDirection (N := Fin 2) :=
  ![{
      curve := su2PauliXCurve
      curve_zero := su2PauliXCurve_zero
      curve_add := su2PauliXCurve_add
      continuous_curve := su2PauliXCurve_continuous
    }, {
      curve := su2PauliYCurve
      curve_zero := su2PauliYCurve_zero
      curve_add := su2PauliYCurve_add
      continuous_curve := su2PauliYCurve_continuous
    }, {
      curve := su2PauliZCurve
      curve_zero := su2PauliZCurve_zero
      curve_add := su2PauliZCurve_add
      continuous_curve := su2PauliZCurve_continuous
    }]

/-- Infinitesimal generators `i σ₁`, `i σ₂`, `i σ₃` of the three concrete
curves. -/
noncomputable def su2PauliGenerator : Fin 3 → Matrix (Fin 2) (Fin 2) Complex :=
  ![!![0, Complex.I; Complex.I, 0],
    !![0, 1; -1, 0],
    !![Complex.I, 0; 0, -Complex.I]]

/-- The standard positive real form on skew-Hermitian `2 × 2` matrices. The
factor `1/2` fixes the Casimir normalization used by the Pauli generators. -/
noncomputable def su2GeneratorInner
    (X Y : Matrix (Fin 2) (Fin 2) Complex) : Real :=
  -(1 / 2 : Real) * (Matrix.trace (X * Y)).re

/-- The three Pauli generators have exactly unit norm and zero mutual inner
products in the standard `SU(2)` normalization. -/
theorem su2PauliGenerator_orthonormal (a b : Fin 3) :
    su2GeneratorInner (su2PauliGenerator a) (su2PauliGenerator b) =
